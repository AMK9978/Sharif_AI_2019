...
public class AI {
    ...
    //boolean[] heroesPanah = {false,false,false,false};
    private int globalAP;
    public ArrayList<OppDetails> oppDetailsList = new ArrayList<>();
    private HashMap<Hero, Integer> healths = new HashMap<>();
    ...
    public class NiceCell implements Comparable<NiceCell> 
    {
        double score = 0;
        Hero hero;
    ...
    public Boolean isOccupied(Cell cell, World world) {
    ...
    public Cell getClosestCellsForDodge(World world, Hero hero, int dodge_board,ArrayList<Cell> BlockedCells , ArrayList<Cell> blocks) {
        ...
        for (Cell cell : cells) {
            if (!hero.getCurrentCell().isInObjectiveZone()) {
                if (!isOccupied(cell, world) && !BlockedCells.contains(cell) && !blocks.contains(cell)
                ...
                if (!isOccupied(cell, world) && !BlockedCells.contains(cell) && !blocks.contains(cell)
                    ...
                    if (hero.getName().equals(HeroName.HEALER)) {
                        niceCell.score = getCellScore(hero, world, cell,
                                null, hero, true,BlockedCells,blocks);
                    } else {
                        niceCell.score = getCellScore(hero, world, cell,
                                null, null, false,BlockedCells,blocks);
                    }
    ...
    public Cell findBestCellAction(World world,Hero hero, Ability ability,Hero[] oppHeroes)
    {
        Cell goodCell = null;
        double score = -1000;
        ...
        //int Hpe=1000;
        //Hero besthero = null;
        for(Hero opp : oppHeroes)
        {
            
            if(opp.getCurrentCell().isInVision() == false)
                continue;
            
//            if(healths.get(opp) < Hpe)
//            {
//                besthero = opp;
//                Hpe = healths.get(opp);
//            }
            list.clear();
            int heroRow = opp.getCurrentCell().getRow();
            int heroCol = opp.getCurrentCell().getColumn();
            double sc;
            for(int i=-area; i<= area; i++)
             {
                for(int j=Math.abs(i)- area; j<= area - Math.abs(i); j++)
                {

                    Cell cell = world.getMap().getCell(heroRow+i ,heroCol+ j);
                    if(world.manhattanDistance(cell, hero.getCurrentCell()) 
                            > ability.getRange() + area)
                        continue;
                    sc = getCellScoreAction(world,hero,ability,cell,oppHeroes,score);
                    if(sc > score)
                    {
                        score = sc;
                        goodCell = cell;
    ...
    private double getCellScoreAction(World world,Hero hero, Ability ability,Cell cell,Hero[] oppHeroes, double maxScore)
    {
    ...
    public int faseleFromObjective(World world , Cell aCell)
    {
    ...
    public double getCellScore(Hero hero, World world, Cell targetCell, Direction direction, Hero Healer,
                                Boolean isHealer,ArrayList<Cell> BlockedCells,ArrayList<Cell> blocks) 
    {
        if (hero.getCurrentHP() <= 0 || targetCell.isWall() 
                || (direction != null && BlockedCells.contains(targetCell))) {
            return -999;
        }
        double score = 0;
        ...
        dodgeReady = dodgeAbility.isReady();
        dodgeRange = dodgeAbility.getRange();
        
        Cell[] objective = world.getMap().getObjectiveZone();
        ArrayList<Cell> nazdik = new ArrayList<>();
        for(int i=0; i<31; i++)
        {
            for(int j=0; j<31; j++)
            {
                Cell tCell = world.getMap().getCell(i, j);
                if(faseleFromObjective(world,tCell) < 3)
                {
                    nazdik.add(tCell);
                }
            }
        ...
        if (name.equals(HeroName.GUARDIAN)) {
            ...
            if (attackReady) {
                ...
                if (motamarkez) {
                    ...
                    if (tHero != null && world.getPathMoveDirections(cell, tHero.getCurrentCell(), BlockedCells).length > 0) {
                        if (direction == world.getPathMoveDirections(cell, tHero.getCurrentCell(), BlockedCells)[0]) {
                    ...
                    if (tHero != null) {
                        if (world.getPathMoveDirections(cell, tHero.getCurrentCell(), BlockedCells).length > 0) {
                            if (direction == world.getPathMoveDirections(cell, tHero.getCurrentCell(), BlockedCells)[0]) {
                                if (tHero.getCurrentCell().isInObjectiveZone() ||
                                        world.manhattanDistance(cell, tHero.getCurrentCell()) < 2) {
                                    score += 3;
                ...
                if (komak != null) {
                    if (world.getPathMoveDirections(cell, komak.getCurrentCell(), BlockedCells).length > 0) {
                        if (world.getPathMoveDirections(cell, komak.getCurrentCell(), BlockedCells)[0] == direction) {
                            score += 1;
                        }
...
//                    }
        ...
        {
            if(powerReady ==false && attackReady==false)
            {
                //now should scape
            ...
            else
            {
                ...
                {
                    Cell goodCell = null;
                    int attack = 0, power = 0,enemyScore = 0;
                    for(Cell aCell : nazdik)
                    {
                        ...
                        if(power+attack > 0)
                        {
                            if( (power*3)+ (attack *2) > enemyScore)
                            {
                    ...
                    if(world.getPathMoveDirections(cell, goodCell).length >0)
                    {
                       if(world.getPathMoveDirections(cell, goodCell)[0] == direction)
                       {
                           score += (power * 3.5) + (attack * 2);
                       }
                ...
                for (Hero oHero : oppHeroes) 
                {
                    ...
                    if (!heroCell.isInVision() || oHero.getCurrentHP() <= 0)
                        continue;
                    ...
                    if(direction != null || oHero.getName().equals(HeroName.GUARDIAN)) 
                    {
                        ...
                        if (world.isInVision(targetCell, heroCell) && attackReady
                            ...
                            score -= (world.manhattanDistance(heroCell, targetCell) - 5) / 2;
                        }
                        if (world.manhattanDistance(heroCell, targetCell) < 5) {
                            score -= (5 - world.manhattanDistance(heroCell, targetCell)) / 2;
                        }
                    }
                }
        }
        ...
        if (name.equals(HeroName.SENTRY)) {
            ...
            for (int i = 0; i < 5; i++) {
                if (world.getPathMoveDirections(cell, goodCell, BlockedCells).length == 0) {
                    blocks.add(goodCell);
                    goodCell = getClosestCells(world, hero).get(0);
                }
            }
            blocks.clear();
            if (!hero.getCurrentCell().isInObjectiveZone() && world.getPathMoveDirections(cell, goodCell, BlockedCells)
                    .length != 0 &&
                    world.getPathMoveDirections(cell, goodCell, BlockedCells)[0] == direction) {
                score += 2.5;
            }
        } else {
            if (targetCell.isInObjectiveZone())
...
//            }
    ...
    private double getCellScore2(Hero hero, World world, Cell targetCell, Direction direction, Hero Healer,  Boolean isHealer) {
...
//        }
    ...
    public double getCellScoreForOpp(Hero hero,OppDetails oppDetails, World world, Cell targetCell, Direction direction, Hero Healer,
                                Boolean isHealer,ArrayList<Cell> BlockedCells,ArrayList<Cell> blocks) 
    {
        if (hero.getCurrentHP() <= 0 || targetCell.isWall() 
                || (direction != null && BlockedCells.contains(targetCell))) {
        ...
        double score = 0;
            ...
            Cell cell = hero.getCurrentCell();
            Hero[] oppHeroes = world.getMyHeroes();
            HeroName name = hero.getName();
            Hero[] myHeroes = world.getOppHeroes();
            int powerRange=0,attackRange=0,dodgeRange;
            Boolean powerReady=false,attackReady=false , dodgeReady = false;
            Ability powerAbility=null,attackAbility=null , dodgeAbility=null;
            switch (name) {
        ...
        powerReady = (oppDetails.powerCoolDown == 0);
        powerRange = powerAbility.getRange();
        attackReady = (oppDetails.attackCoolDown == 0);
        attackRange = attackAbility.getRange();
        dodgeReady = (oppDetails.dodgeCoolDown == 0);
        dodgeRange = dodgeAbility.getRange();
        ...
        if (name.equals(HeroName.GUARDIAN)) {
            ...
            if (attackReady) {
                Boolean motamarkez = false;
                for (Hero dHero : myHeroes) {
                    if (dHero.getCurrentHP() == 0)
                        continue;
                    int guardians = 0;
                    for (int i = 0; i < world.getMyHeroes().length; i++) {
                        if (world.getMyHeroes()[i].getName().equals(HeroName.GUARDIAN)) {
                            guardians++;
                        }
                    }
                    if (dHero.getName().equals(HeroName.GUARDIAN) && (guardians == 1 || (dHero.getCurrentHP()
                    ...
                    if (dHero.getCurrentHP() < minHP) {
                        if (world.manhattanDistance(dHero.getCurrentCell(), hero.getCurrentCell()) > 2) {
                            ...
                            for (int i = 0; i < 4; i++) {
                                if (world.getMyHeroes()[i].getCurrentHP() > 0) {
                                    if (world.manhattanDistance(hero.getCurrentCell(), world.getMyHeroes()[i].
                                            getCurrentCell()) < world.manhattanDistance(dHero.getCurrentCell(),
                                            hero.getCurrentCell())) {
                                        if (world.getMyHeroes()[i].getCurrentCell().isInObjectiveZone()) {
                                            motamarkez = true;
                                            tHero = world.getMyHeroes()[i];
                                            flag = false;
                                            dHero = tHero;
                                        }
                                    }
...
//            }
    ...
    public ArrayList<Cell> getClosestCells(World world, Hero hero) {
        Cell[] cells = world.getMap().getObjectiveZone();
    ...
    private void finding_good_cell_to_move(Hero[] My_heroes, Hero hero, World world) {
        ...
        //dont move
        if (targetCell.isInObjectiveZone()) {
            score = getCellScore2(hero, world, targetCell, null, Healer, isHealer);
            score += 3.0 / hero.getMoveAPCost();
...
//                score -= 1.5;
        ...
        if (world.getMap().isInMap(cell.getRow() - 1, cell.getColumn()) &&
            ...
            score = getCellScore2(hero, world, targetCell
                    , Direction.UP, Healer, isHealer);
        ...
        if (world.getMap().isInMap(cell.getRow() + 1, cell.getColumn()) &&
            ...
            targetCell = world.getMap().getCell(cell.getRow() + 1, cell.getColumn());
            score = getCellScore2(hero, world, targetCell, Direction.DOWN, Healer, isHealer);
            Cell goodCell = getClosestCells(world, hero).get(0);
        ...
        if (world.getMap().isInMap(cell.getRow(), cell.getColumn() - 1) &&
            ...
            score = getCellScore2(hero, world, targetCell
                    , Direction.LEFT, Healer, isHealer);
            niceTargetRow = cell.getRow();
        ...
        if (world.getMap().isInMap(cell.getRow(), cell.getColumn() + 1) &&
            ...
            targetCell = world.getMap().getCell(cell.getRow(), cell.getColumn() + 1);
            score = getCellScore2(hero, world, targetCell, Direction.RIGHT
                    , Healer, isHealer);
            NiceCell nice_cell = new NiceCell();
            niceTargetRow = cell.getRow();
            niceTargetColl = cell.getColumn() + 1;
    ...
    public void preProcess(World world)
    {
        ...
        for(int i=0; i<31; i++)
        {
            for(int j=0; j<31; j++)
            {
                ...
                if(faseleFromObjective(world,cell) < 6)
                {
                    nazdik.add(cell);
                }
            }
        ...
        Cell[] objective = world.getMap().getObjectiveZone();
        int score = 0;
        for(Cell cell : objective)
        {
            for(Cell aCell : nazdik)
            {
                if(world.isInVision(cell, aCell))
                    score++;
            }
        }
        System.out.println("map score:" +score);
//        if (dispersion1  + dispersion2> delta) {
//            extraSentry = true;
//        }
    ...
    public void moveTurn(World world) {
        ...
        for (int i = 0; i < world.getMyCastAbilities().length; i++) {
            System.out.println(world.getMyCastAbilities()[i].getAbilityName() + ",id:" + world.getMyCastAbilities()[i].getCasterId());
        }
        int O_o = (int) System.currentTimeMillis();
        Hero My_hero;
        if (pick_period == 0) {
            for (int i = 0; i < 4; i++) {
                System.out.println("name:" + world.getMyHeroes()[i].getName() 
                        + ",id:" + world.getMyHeroes()[i].getId());
                
                OppDetails aOppDetails = new OppDetails(world.getMyHeroes()[i].getId());
                oppDetailsList.add(0,aOppDetails);
            }
    ...
    public void actionTurn(World world) 
    {
        ...
        for (Hero hero : heroes) 
        {
            ...
            if (flag) 
            {
                if (hero.getName().equals(HeroName.SENTRY)) 
                {
                    if (hero.getAbility(AbilityName.SENTRY_RAY).isReady() && globalAP >= hero.getAbility(AbilityName.SENTRY_RAY).getAPCost() ) 
                    {
                        Cell cell = findBestCellAction(world, hero, hero.getAbility(AbilityName.SENTRY_RAY)
                                , Opp_Heroes);
                        if(cell != null)
                        {
                            globalAP -= hero.getAbility(AbilityName.SENTRY_RAY).getAPCost();
                            world.castAbility(hero, AbilityName.SENTRY_RAY, cell);
                            for(Hero badHero : list)
                                healths.put(badHero, healths.get(badHero) - hero.getAbility(AbilityName.SENTRY_RAY).getPower());
                            System.out.println("sentry attacked and end:" + cell);
                        }
                    } 
                    else if(hero.getAbility(AbilityName.SENTRY_ATTACK).isReady()&& globalAP >= hero.getAbility(AbilityName.SENTRY_ATTACK).getAPCost())
                    {
                        Cell cell = findBestCellAction(world, hero, hero.getAbility(AbilityName.SENTRY_ATTACK)
                                , Opp_Heroes);
                        if(cell != null)
                        {
                            globalAP -= hero.getAbility(AbilityName.SENTRY_ATTACK).getAPCost();;
                            world.castAbility(hero, AbilityName.SENTRY_ATTACK, cell);
                            for(Hero badHero : list)
                                healths.put(badHero, healths.get(badHero) - hero.getAbility(AbilityName.SENTRY_ATTACK).getPower());
                            System.out.println("sentry attacked and end:" + cell);
                        }
                    }
                }
                ...
                {
                    if(hero.getAbility(AbilityName.BLASTER_BOMB).isReady() && globalAP >= hero.getAbility(AbilityName.BLASTER_BOMB).getAPCost())
                    {
                        System.out.println("Blaster attack is ready and turn:" + world.getCurrentTurn());
                        Cell cell = findBestCellAction(world, hero, hero.getAbility(AbilityName.BLASTER_BOMB)
                                , Opp_Heroes);
                        if(cell != null)
                        {
                            globalAP -= hero.getAbility(AbilityName.BLASTER_BOMB).getAPCost();
                            world.castAbility(hero,AbilityName.BLASTER_BOMB , cell);
                            for(Hero badHero : list)
                                healths.put(badHero, healths.get(badHero) - hero.getAbility(AbilityName.BLASTER_BOMB).getPower());
                            System.out.println("sentry attacked and end:" + cell);
                        }
                    }
                    else if (hero.getAbility(AbilityName.BLASTER_ATTACK).isReady()&& globalAP >= hero.getAbility(AbilityName.BLASTER_ATTACK).getAPCost()) 
                    {
                        System.out.println("Blaster attack is ready and turn:" + world.getCurrentTurn());
                        Cell cell = findBestCellAction(world, hero, hero.getAbility(AbilityName.BLASTER_ATTACK)
                                , Opp_Heroes);
                        if(cell != null)
                        {
                            globalAP -= hero.getAbility(AbilityName.BLASTER_ATTACK).getAPCost();
                            world.castAbility(hero,AbilityName.BLASTER_ATTACK , cell);
                            for(Hero badHero : list)
                                healths.put(badHero, healths.get(badHero) - hero.getAbility(AbilityName.BLASTER_ATTACK).getPower());
                            System.out.println("sentry attacked and end:" + cell);
                        }
                    }
                ...
                {
                    if (hero.getAbility(AbilityName.GUARDIAN_ATTACK).isReady() && globalAP >= hero.getAbility(AbilityName.GUARDIAN_ATTACK).getAPCost()) {
                        System.out.println("Guardian attack is ready and turn:" + world.getCurrentTurn());
                        Cell cell = findBestCellAction(world, hero, hero.getAbility(AbilityName.GUARDIAN_ATTACK)
                                , Opp_Heroes);
                        if(cell != null)
                        {
                            globalAP -= hero.getAbility(AbilityName.GUARDIAN_ATTACK).getAPCost();
                            world.castAbility(hero,AbilityName.GUARDIAN_ATTACK , cell);
                            for(Hero badHero : list)
                                healths.put(badHero, healths.get(badHero) - hero.getAbility(AbilityName.GUARDIAN_ATTACK).getPower());
                            System.out.println("sentry attacked and end:" + cell);
                        }
                    }
                    ...
                    // Healer
                    if (hero.getAbility(AbilityName.HEALER_ATTACK).isReady() && globalAP >= hero.getAbility(AbilityName.SENTRY_RAY).getAPCost()) {
                        Cell cell = findBestCellAction(world, hero, hero.getAbility(AbilityName.HEALER_ATTACK)
                                , Opp_Heroes);
                        if(cell != null)
                        {
                            globalAP -= hero.getAbility(AbilityName.HEALER_ATTACK).getAPCost();
                            world.castAbility(hero,AbilityName.HEALER_ATTACK , cell);
                            for(Hero badHero : list)
                                healths.put(badHero, healths.get(badHero) - hero.getAbility(AbilityName.HEALER_ATTACK).getPower());
                            System.out.println("sentry attacked and end:" + cell);
                        }
                    }
            ...
            if (!flag || (!hero.getCurrentCell().isInObjectiveZone())) {
                if (!flag) {
                            ...
                            for (int j = 0; j < 31; j++) {
                                if (!isOccupied(world.getMap().getCell(i, j), world) &&
                                    ...
                                    niceCell.score = getCellScore(hero, world, world.getMap().getCell(i, j),
                                            null, null, false,Blocked_Cells,blocks);
                                    niceCell.direction = null;
                                    niceCell.hero = hero;
                    ...
                    if (hero.getName().equals(HeroName.SENTRY)) {
                        // 3
                        Cell des = getClosestCellsForDodge(world, hero, 3,Blocked_Cells,blocks);
                        if (des == null) {
                            des = getClosestCellsForDodge(world, hero, 2,Blocked_Cells,blocks);
                            if (des == null) {
                    ...
                    if (hero.getName().equals(HeroName.BLASTER)) {
                        // 4
                        Cell des = getClosestCellsForDodge(world, hero, 4,Blocked_Cells,blocks);
                        if (des == null) {
                            des = getClosestCellsForDodge(world, hero, 3,Blocked_Cells,blocks);
                            if (des == null) {
                                Random random = new Random();
                                des = world.getMap().getObjectiveZone()[Math.abs(random.nextInt() % 20)];
                    ...
                    if (hero.getName().equals(HeroName.GUARDIAN)) {
                        // 2
                        Cell des = getClosestCellsForDodge(world, hero, 2,Blocked_Cells,blocks);
                        if (des == null) {
                            des = getClosestCellsForDodge(world, hero, 1,Blocked_Cells,blocks);
                            if (des == null) {
                                Random random = new Random();
                                des = world.getMap().getObjectiveZone()[Math.abs(random.nextInt() % 20)];
            ...
            for (int j = 0; j < world.getOppHeroes().length; j++) {
                ...
                if (world.manhattanDistance(myHero.getCurrentCell(), Opphero.getCurrentCell()) <= 7 &&
                    ...
                    if (myHero.getCurrentHP() < 150) {
                        world.castAbility(myHero, AbilityName.GUARDIAN_DODGE, getClosestCellsForDodge(world, myHero, 2,Blocked_Cells,blocks));
                    }
                    world.castAbility(myHero, AbilityName.BLASTER_DODGE, getClosestCellsForDodge(world, myHero, 4,Blocked_Cells,blocks));
        ...
        if (I_m_Guardian != null) {
            ...
            world.castAbility(I_m_Guardian, AbilityName.GUARDIAN_DODGE, getClosestCellsForDodge(world, I_m_Guardian, 2,Blocked_Cells,blocks));
        }
