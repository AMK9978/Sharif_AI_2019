...
```
// ArrayListcellArrayList = new ArrayList<>();
Cell minCell = null;
int min = 9999;
for (Cell cell : cells) {
int i;
...
private double getCellScore(Hero hero, World world, Cell targetCell ,Direction direction, Hero Healer, Boolean isHealer) {
    ...
    if(world.getPathMoveDirections(cell, goodCell)[0] == direction)
        score +=3;
    if(name == HeroName.BLASTER || name == HeroName.HEALER)
    {
        ...
        for(Cell aCell : objective)
        {
            ...
            if(notVis)
            {
                if(direction == world.getPathMoveDirections(cell, aCell)[0])
                    score += 2;
                break;
            }
        }
    ...
    for (Hero oppHero : oppHeroes) {
        ...
        if ((hero.getName() != HeroName.SENTRY || hero.getAbility(AbilityName.SENTRY_RAY).isReady()==false) 
            ...
            score -= 1;
                ...
                && (hero.getName() != HeroName.BLASTER || hero.getAbility(AbilityName.BLASTER_BOMB).isReady()==false)) {
...
public void preProcess(World world) {
    ...
    ArrayList<Cell> wallCell = new ArrayList<>();
    int n=0;
    for (int i = 0; i < 31; i++) {
        for (int j = 0; j < 31; j++) {
            if (cells[i][j].isWall())
                wallCell.add(cells[i][j]); 
                //dis.add(world.manhattanDistance(i, j, 0, 0));
        }
    }
    for(Cell aCell : wallCell)
    {
        for(Cell bCell : wallCell)
        {
            if(aCell != bCell)
                dis.add(world.manhattanDistance(aCell.getRow(),aCell.getColumn(), bCell.getRow(), bCell.getColumn()));
        }
    }
    int delta = 250;
        ...
        if (hero.getName().equals(HeroName.SENTRY)) {
            ...
            if (hero.getAbility(AbilityName.SENTRY_RAY).isReady()) {
                ...
                for (Hero oppHero : Opp_Heroes) {
                    if (world.isInVision(hero_cell, oppHero.getCurrentCell()) && oppHero.getCurrentHP() < minHP)
                        goodOpp = oppHero;
            ...
            if (hero.getAbility(AbilityName.BLASTER_BOMB).isReady()) {
                ...
                for (Hero oppHero : Opp_Heroes) {
                    if (world.manhattanDistance(hero_cell, oppHero.getCurrentCell()) 
                            ...
                            hero.getAbility(AbilityName.BLASTER_BOMB).getRange() && oppHero.getCurrentHP() < minHP) {
                        goodOpp = oppHero;
                    }
                }
                if(goodOpp != null)
                ...
                for (Hero oppHero : Opp_Heroes) {
                    ...
                    if (world.manhattanDistance(hero_cell, oppHero.getCurrentCell()) 
                            ...
                            hero.getAbility(AbilityName.BLASTER_ATTACK).getRange() && oppHero.getCurrentHP() < minHP) {
            ...
            if (hero.getAbility(AbilityName.GUARDIAN_ATTACK).isReady()) {
                for (Cell Opp_cell : Opp_cells) {
                    if (world.manhattanDistance(hero_cell, Opp_cell) - hero.getAbility(AbilityName.GUARDIAN_ATTACK).getAreaOfEffect()<=
            ...
            if (hero.getAbility(AbilityName.GUARDIAN_FORTIFY).isReady()) {
                for (Hero hero1 : world.getMyHeroes()) {
                    ...
                    if (hero1.getCurrentHP() < hero1.getMaxHP() && world.manhattanDistance(hero_cell, des) - hero.getAbility(AbilityName.GUARDIAN_FORTIFY).getAreaOfEffect()<=
            ...
            if (hero.getAbility(AbilityName.HEALER_ATTACK).isReady()) {
                for (Cell Opp_cell : Opp_cells) {
                    if (world.manhattanDistance(hero_cell, Opp_cell) - hero.getAbility(AbilityName.HEALER_ATTACK).getAreaOfEffect()<=
            ...
            if (hero.getAbility(AbilityName.HEALER_HEAL).isReady()) {
                for (Hero hero1 : world.getMyHeroes()) {
                    Cell des = hero1.getCurrentCell();
                    if (hero1.getCurrentHP() < hero1.getMaxHP() && world.manhattanDistance(hero_cell, des) - hero.getAbility(AbilityName.HEALER_HEAL).getAreaOfEffect()<=
