...
//            }
    ...
    //finding good cell for sentry to go there for using its ray
    private Cell find_good_cell_for_sentry_to_go(World world){
        ...
        for(Cell[] cell:cells){
            for(Cell c :cell){
                ...
                for(Cell objective:objectiveZone){
                    if(world.isInVision(c,objective) && world.manhattanDistance(c,objective)>=8)
                        score++;
                }
    ...
    Cell find_good_op_for_blaster(World world, Hero myHero) {
        ...
        for (int row = myCell.getRow() - range; row <= myCell.getRow() + range; row++) {
            for (int col = myCell.getColumn() - range; col <= myCell.getColumn() + range; col++) {
                ...
                if (world.getMap().isInMap(row,col)&& world.manhattanDistance(cell, myCell) <= myHero.getAbility(AbilityName.BLASTER_BOMB).getRange())
                    cells.add(cell);
            }
        }
        ...
        System.out.println("max score for good cell for blaster bomb"+maxScore);
        if (maxScore == 1) {
            int minHp = 1000;
            for (Hero op : opHeroes) {
            ...
            if (flag) {
                if (hero.getName().equals(HeroName.SENTRY)) {
                    ...
                    if (hero.getAbility(AbilityName.BLASTER_BOMB).isReady()) {
                        ...
                        if (targetCell != null) {
                            System.out.println("target cell for my blaster to throw a bomb is: ("+targetCell.getRow()+","+
